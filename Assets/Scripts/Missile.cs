...
public class Missile : MonoBehaviour
{
    ...
    void Detonate()
    {
        ...
        foreach (Collider c in nearby)
        {
            ...
            Rigidbody foundRb = root.GetComponentInChildren<Rigidbody>();
            Damageable foundDamagable = root.GetComponentInChildren<Damageable>();
            if (foundRb != null) { foundRb.AddExplosionForce(explosiveForce, transform.position, detonateRange); }
            if (foundDamagable != null)
            {
                float damage = explosiveForce * (1 - Vector3.Distance(transform.position, foundDamagable.transform.position) / damageRange);
